...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    var treatCircles = chart().focus.selectAll('treatment-dot').data(client.ddata.treatments.filter(function(treatment) {
      ...
      return notCarbsOrInsulin && treatment.duration === undefined && notTempOrProfile && notOpenAPSSpam;
    }));
    ...
    var durationTreatments = client.ddata.treatments.filter(function(treatment) {
      return !treatment.carbs && !treatment.insulin && treatment.duration !== undefined &&
        !_.includes(['Temp Basal', 'Profile Switch', 'Combo Bolus', 'Temporary Target', 'Sensor Start', 'Sensor Change', 'Sensor Stop', 'BG Check', 'Insulin Change', 'Site Change'], treatment.eventType);
    });
    ...
    gs.append('text')
      ...
      .style('font-size', 15)
      .attr('fill', 'white')
      .attr('text-anchor', function(d) { return d.duration === 0 ? 'left' : 'middle'} )
      .attr('dy', '.35em')
      .attr('transform', treatmentTextTransform)
      .text(function(d) {
        if (d.eventType === 'Temporary Target') {
